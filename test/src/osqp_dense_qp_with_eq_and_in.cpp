                  ...
                  "constraints and increasing dimension using the API")
{
  ...
  proxqp::utils::rand::set_seed(1);
  // for (proxqp::isize dim = 10; dim < 1000; dim += 100) {
  // for (proxqp::isize dim = 10; dim < 110; dim += 100) {
  proxqp::isize dim = 10;
  {
    ...
    std::cout << "qp_random.C" << std::endl << qp_random.C << std::endl;
    std::cout << "qp_random.l" << std::endl << qp_random.l << std::endl;
    std::cout << "qp_random.u" << std::endl << qp_random.u << std::endl;
    pod::QP<T> qp{ dim, n_eq, n_in }; // creating QP object
    qp.settings.eps_abs = eps_abs;
    qp.settings.eps_rel = 0;
    qp.settings.default_mu_eq = T(1.E-2);
