  ...
  for (auto const& dims : { // proxsuite::linalg::veg::tuplify(50, 0, 0),
    ...
    proxsuite::proxqp::Results<T> results =
      proxsuite::proxqp::sparse::solve<T, I>(
        H, g, A, b, C, u, l, std::nullopt, std::nullopt, std::nullopt, eps_abs);

    T dua_res = proxqp::dense::infty_norm(
      H.selfadjointView<Eigen::Upper>() * results.x + g +
      A.transpose() * results.y + C.transpose() * results.z);
    T pri_res = std::max(proxqp::dense::infty_norm(A * results.x - b),
                         proxqp::dense::infty_norm(
                           sparse::detail::positive_part(C * results.x - u) +
                           sparse::detail::negative_part(C * results.x - l)));
                  ...
                  "inequality constraints: test solve with different rho value")
{
  ...
  for (auto const& dims : { // proxsuite::linalg::veg::tuplify(50, 0, 0),
    ...
    auto u = (l.array() + 10).matrix().eval();
    proxsuite::proxqp::Results<T> results =
      proxsuite::proxqp::sparse::solve<T, I>(H,
                                             g,
                                             A,
                                             b,
                                             C,
