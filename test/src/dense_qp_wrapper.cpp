...
{
  std::cout
    << "---ProxQP: testing sparse random strongly convex qp with inequality "
       "constraints "
       "and empty equality constraints---"
    << std::endl;
  ...
  proxqp::dense::QP<T> qp2{ dim, n_eq, n_in }; // creating QP object
  ...
  proxqp::dense::QP<T> qp3{ dim, n_eq, n_in }; // creating QP object
  ...
  "inequality constraints: test update A")
{
  ...
  qp_random.A = common::utils::rand::sparse_matrix_rand_not_compressed<T>(
  ...
  "inequality constraints: test update C")
{
  ...
  qp_random.C = common::utils::rand::sparse_matrix_rand_not_compressed<T>(
  ...
  "inequality constraints: test update u")
{
  ...
  std::cout << "testing updating b" << std::endl;
  auto x_sol = common::utils::rand::vector_rand<T>(dim);
  auto delta = common::utils::Vec<T>(n_in);
  ...
  "inequality constraints: test update g")
{
  ...
  auto g = common::utils::rand::vector_rand<T>(dim);
...
DOCTEST_TEST_CASE(
  "ProxQP: sparse random strongly convex qp with equality and inequality "
  "constraints: test update H and A and b and u and l")
{
  ...
  std::cout << "testing updating b" << std::endl;
  qp_random.H =
    common::utils::rand::sparse_positive_definite_rand_not_compressed<T>(
  ...
  "inequality constraints: test with warm start with previous result")
{
  ...
  // << std::endl;
  qp2.ruiz.scale_primal_in_place({ common::from_eigen, x });
  qp2.ruiz.scale_dual_in_place_eq({ common::from_eigen, y });
  qp2.ruiz.scale_dual_in_place_in({ common::from_eigen, z });
  // std::cout << "after scaling x " << x <<  " qp.results.x " << qp.results.x
  // << std::endl;
  qp2.solve(x, y, z);
...
TEST_CASE("ProxQP: :dense: Test A update for different initial guess")
{
  ...
  CHECK(dua_res <= eps_abs);
  CHECK(pri_res <= eps_abs);
  auto new_A = common::utils::rand::sparse_matrix_rand_not_compressed<T>(
    n_eq, dim, sparsity_factor);
...
TEST_CASE(
  "ProxQP: :dense: Test g update for different warm start with previous "
  "result option")
{
...
TEST_CASE("ProxQP: :dense: check ordering of z when there are box constraints")
{
  ...
  for (isize i = 0; i < n_test; i++) {
    ...
    proxqp::dense::QP<T> qp_compare(dim, n_eq, dim, false);
    qp_compare.settings.eps_abs = eps_abs;
    qp_compare.settings.eps_rel = 0;
...
TEST_CASE("ProxQP: :dense: test primal infeasibility solving")
{
  ...
  for (isize i = 0; i < 20; ++i) {
    ...
    utils::Vec<T> rhs_dim(dim);
    utils::Vec<T> rhs_n_eq(n_eq);
    rhs_n_eq.setOnes();
    utils::Vec<T> rhs_n_in(n_in);
    rhs_n_in.setOnes();
    rhs_dim.noalias() =
...
DOCTEST_TEST_CASE(
  "ProxQP: check that model.is_valid function for symmetric matrices "
          ...
          "eigenvalue with power iteration")
{
  ...
  common::dense::power_iteration(H, dw, rhs, err_v, 1.E-6, 10000);
          ...
          "inequality constraints: test PrimalLDLT backend mu update")
{
  ...
  std::cout << "---ProxQP: testing sparse random strongly convex qp with"
               "inequality constraints: test PrimalLDLT backend mu update---"
            << std::endl;
  double sparsity_factor = 1;
  common::utils::rand::set_seed(1);
  ...
  proxqp::dense::QP<T> qp{
    dim, n_eq, n_in, false, HessianType::Dense, DenseBackend::PrimalLDLT
  }; // creating QP object
  T eps_abs = T(1e-7);
  qp.settings.eps_abs = eps_abs;
  qp.settings.eps_rel = 0;
