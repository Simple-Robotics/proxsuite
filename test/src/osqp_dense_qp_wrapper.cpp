...
// DOCTEST_TEST_CASE(
//   "ProxQP::dense: sparse random strongly convex qp with inequality
//   constraints" "and empty equality constraints")
// {
...
//   +
//              helpers::negative_part(qp_random.C * qp.results.x -
//              qp_random.l))
//               .lpNorm<Eigen::Infinity>();
...
//   qp.update(qp_random.H, nullopt, nullopt, nullopt, nullopt, nullopt,
//   nullopt);
...
//   // std::cout << "------using API solving qp with dim after updating: " <<
//   dim
//   //  << " neq: " << n_eq << " nin: " << n_in << std::endl;
//   // std::cout << "primal residual: " << pri_res << std::endl;
//   // std::cout << "dual residual: " << dua_res << std::endl;
...
//   // conter factual check with another QP object starting at the updated
//   model osqp::dense::QP<T> qp2{ dim, n_eq, n_in }; // creating QP object
//   qp2.settings.eps_abs = eps_abs;
//   qp2.settings.eps_rel = 0;
//   qp2.init(qp_random.H,
//            qp_random.g,
...
//   // std::cout << "------ conter factual check with another QP object
//   starting
//   // at "
//   //  "the updated model : "
...
//   //  << std::endl;
//   // std::cout << "setup timing " << qp2.results.info.setup_time << " solve
//   time
//   // "
...
//   qp.update(nullopt, nullopt, qp_random.A, nullopt, nullopt, nullopt,
//   nullopt);
...
//   qp.update(nullopt, nullopt, nullopt, nullopt, qp_random.C, nullopt,
//   nullopt);
...
//   qp.update(nullopt, nullopt, nullopt, qp_random.b, nullopt, nullopt,
//   nullopt);
...
//   qp.update(nullopt, nullopt, nullopt, nullopt, nullopt, nullopt,
//   qp_random.u);
...
//   qp_random.g = g;
//   qp.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt);
...
//   // std::cout
//   //  << "---testing sparse random strongly convex qp with equality and "
//   //  "inequality constraints: test with equality constrained initial
//   guess---"
//   //  << std::endl;
//   double sparsity_factor = 0.15;
//   T eps_abs = T(1e-5); // ADMM only (high precision)
  ...
  "inequality constraints: test with warm start with previous result")
{
  ...
  auto y = qp.results.y;
  auto z = qp.results.z;
  // // std::cout << "after scaling x " << x <<  " qp.results.x " <<
  // qp.results.x
  // << std::endl;
  ...
  //            qp_random.C.transpose() * qp.results.z)
  //             .lpNorm<Eigen::Infinity>();
  // // std::cout << "------using API solving qp with dim with qp after warm
  // start
  // // "
...
//   DOCTEST_CHECK(pri_res <= eps_abs);
//   DOCTEST_CHECK(dua_res <= eps_abs);
//   // std::cout << "------using API solving qp with dim with cold start
//   option: "
//   //  << dim << " neq: " << n_eq << " nin: " << n_in << std::endl;
//   // std::cout << "primal residual: " << pri_res << std::endl;
//   // std::cout << "dual residual: " << dua_res << std::endl;
...
//   // std::cout
//   //  << "---testing sparse random strongly convex qp with equality and "
//   //  "inequality constraints: test equilibration options at
//   initialization---"
//   //  << std::endl;
//   double sparsity_factor = 0.15;
//   T eps_abs = T(1e-5); // ADMM only (high precision)
...
//   DOCTEST_CHECK(pri_res <= eps_abs);
//   DOCTEST_CHECK(dua_res <= eps_abs);
//   // std::cout << "------using API solving qp without preconditioner
//   derivation:
//   // "
//   //  << dim << " neq: " << n_eq << " nin: " << n_in << std::endl;
//   // std::cout << "primal residual: " << pri_res << std::endl;
...
//             nullopt,
//             nullopt,
//             true); // rederive preconditioner with previous options, i.e.,
//             redo
...
//              qp_random.C.transpose() * qp2.results.z)
//               .lpNorm<Eigen::Infinity>();
//   // std::cout << "------using API solving qp with preconditioner derivation
//   and
//   // "
//   //  "another object QP: "
//   //  << dim << " neq: " << n_eq << " nin: " << n_in << std::endl;
...
//   // std::cout << "Test with equality constrained initial guess" <<
//   std::endl;
//   // std::cout << "dirty workspace before any solving: " << qp.work.dirty
//   //  << std::endl;
...
// TEST_CASE("ProxQP::dense: sparse random strongly convex qp with equality and
// "
//           "inequality constraints: test multiple solve at once with cold
//           start " "initial guess")
// {
...
// TEST_CASE("ProxQP::dense: sparse random strongly convex qp with equality and
// "
//           "inequality constraints: test multiple solve at once with warm
//           start")
// {
...
//   // std::cout << "Test with warm start and first solve with no initial
//   guess"
//   //  << std::endl;
//   // std::cout << "dirty workspace before any solving: " << qp.work.dirty
//   //  << std::endl;
...
//   qp.settings.eps_rel = 0;
//   qp2.settings.initial_guess = InitialGuessStatus::WARM_START;
//   // std::cout << "dirty workspace for qp2 : " << qp2.work.dirty <<
//   std::endl; qp2.solve(qp.results.x, qp.results.y, qp.results.z); pri_res =
//   std::max(
//     (qp_random.A * qp2.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp2.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp2.results.x - qp_random.l))
...
// TEST_CASE("ProxQP::dense: sparse random strongly convex qp with equality and
// "
//           "inequality constraints: test update and multiple solve at once
...
//   // std::cout << "dirty workspace after update : " << qp.work.dirty <<
//   // std::endl;
//   qp.solve();
//   pri_res = std::max(
//     (qp_random.A * qp.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
...
//   //  << std::endl;
//   // std::cout << "setup timing " << qp3.results.info.setup_time << " solve
//   time
//   // "
//   //  << qp3.results.info.solve_time << std::endl;

//   osqp::dense::QP<T> qp4(dim, n_eq, n_in);
...
//   //  << std::endl;
//   // std::cout << "setup timing " << qp4.results.info.setup_time << " solve
//   time
//   // "
...
//   //  << std::endl;
//   // std::cout << "setup timing " << qp5.results.info.setup_time << " solve
//   time
//   // "
...
//   qp_random.g = utils::rand::vector_rand<T>(dim);
//   qp.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt); qp.solve(); pri_res = std::max(
//     (qp_random.A * qp.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp.results.x - qp_random.l))
//       .lpNorm<Eigen::Infinity>());
...
//   CHECK(dua_res <= eps_abs);
//   CHECK(pri_res <= eps_abs);
//   qp2.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt); qp2.solve(); pri_res = std::max(
//     (qp_random.A * qp2.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp2.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp2.results.x - qp_random.l))
//       .lpNorm<Eigen::Infinity>());
...
// //   CHECK(dua_res <= eps_abs); // Fail here CHECK( 412.403 <= 1e-05 )
// //   CHECK(pri_res <= eps_abs); // Fail here CHECK( 0.0804118 <= 1e-05 )
//   // std::cout << "--n = " << dim << " n_eq " << n_eq << " n_in " << n_in
//   //  << std::endl;
//   // std::cout << "; dual residual " << dua_res << "; primal residual " <<
//   // pri_res
...
//   CHECK(dua_res <= eps_abs);
//   CHECK(pri_res <= eps_abs);
//   qp3.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt); qp3.solve(); pri_res = std::max(
//     (qp_random.A * qp3.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp3.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp3.results.x - qp_random.l))
//       .lpNorm<Eigen::Infinity>());
...
//   CHECK(dua_res <= eps_abs);
//   CHECK(pri_res <= eps_abs);
//   qp4.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt); qp4.solve(); pri_res = std::max(
//     (qp_random.A * qp4.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp4.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp4.results.x - qp_random.l))
//       .lpNorm<Eigen::Infinity>());
...
//   CHECK(dua_res <= eps_abs);
//   CHECK(pri_res <= eps_abs);
//   qp5.update(nullopt, qp_random.g, nullopt, nullopt, nullopt, nullopt,
//   nullopt); qp5.solve(); pri_res = std::max(
//     (qp_random.A * qp5.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//     (helpers::positive_part(qp_random.C * qp5.results.x - qp_random.u) +
//      helpers::negative_part(qp_random.C * qp5.results.x - qp_random.l))
//       .lpNorm<Eigen::Infinity>());
...
//              (helpers::positive_part(qp_random.C * qp.results.x -
//              qp_random.u) +
//               helpers::negative_part(qp_random.C * qp.results.x -
//               qp_random.l))
//                .lpNorm<Eigen::Infinity>());
...
//      new_A.transpose() * qp.results.y + qp_random.C.transpose() *
//      qp.results.z)
//       .lpNorm<Eigen::Infinity>();
...
// TEST_CASE("ProxQP::dense: Test g update for different warm start with
// previous "
//           "result option")
// {
...
//      qp_random.C.transpose() * qp.results.z)
//       .lpNorm<Eigen::Infinity>();
//   // CHECK(dua_res <= eps_abs); // Fail here CHECK( 422.267 <= 1e-05 )
//   // CHECK(pri_res <= eps_abs); // Fail here CHECK( 0.0827395 <= 1e-05 )
//   // std::cout << "--n = " << dim << " n_eq " << n_eq << " n_in " << n_in
//   //  << std::endl;
//   // std::cout << "; dual residual " << dua_res << "; primal residual " <<
//   // pri_res
...
//       .lpNorm<Eigen::Infinity>());
//   dua_res =
//     (qp_random.H * qp2.results.x + g + qp_random.A.transpose() *
//     qp2.results.y +
//      qp_random.C.transpose() * qp2.results.z)
//       .lpNorm<Eigen::Infinity>();
//   CHECK(dua_res <= eps_abs);
...
//               .lpNorm<Eigen::Infinity>();
//   DOCTEST_CHECK(pri_res <= eps_abs);
//   // DOCTEST_CHECK(dua_res <= eps_abs); // Fail here CHECK( 2.57172e-05 <=
//   1e-05 )
//   // conter factual check with another QP object starting at the updated
//   model osqp::dense::QP<T> qp2{ dim, n_eq, n_in }; // creating QP object
...
//   DOCTEST_CHECK(std::abs(T(1) / mu_eq - qp2.results.info.mu_eq_inv)
//   <= 1.E-9); qp2.solve(); DOCTEST_CHECK(std::abs(mu_eq -
//   qp2.settings.default_mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(mu_eq -
//   qp2.results.info.mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(T(1) / mu_eq -
//   qp2.results.info.mu_eq_inv) <= 1.E-9);
...
//   // conter factual check with another QP object starting at the updated
//   model osqp::dense::QP<T> qp3{ dim, n_eq, n_in }; // creating QP object
...
//   DOCTEST_CHECK(std::abs(T(1) / mu_eq - qp3.results.info.mu_eq_inv)
//   <= 1.E-9); qp3.solve(); DOCTEST_CHECK(std::abs(rho -
//   qp3.settings.default_rho) <= 1.E-9); DOCTEST_CHECK(std::abs(rho -
//   qp3.results.info.rho) <= 1.E-9); DOCTEST_CHECK(std::abs(mu_eq -
//   qp3.settings.default_mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(mu_eq -
//   qp3.results.info.mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(T(1) / mu_eq -
//   qp3.results.info.mu_eq_inv) <= 1.E-9);
...
//     qp2.settings.default_mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(mu_eq -
//     qp2.results.info.mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(T(1) / mu_eq -
//     qp2.results.info.mu_eq_inv) <= 1.E-9); pri_res = std::max(
//       (qp_random.A * qp2.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//       (helpers::positive_part(qp_random.C * qp2.results.x - qp_random.u) +
//        helpers::negative_part(qp_random.C * qp2.results.x - qp_random.l))
//         .lpNorm<Eigen::Infinity>());
...
//     qp3.settings.default_mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(mu_eq -
//     qp3.results.info.mu_eq) <= 1.E-9); DOCTEST_CHECK(std::abs(T(1) / mu_eq -
//     qp3.results.info.mu_eq_inv) <= 1.E-9); pri_res = std::max(
//       (qp_random.A * qp3.results.x - qp_random.b).lpNorm<Eigen::Infinity>(),
//       (helpers::positive_part(qp_random.C * qp3.results.x - qp_random.u) +
//        helpers::negative_part(qp_random.C * qp3.results.x - qp_random.l))
//         .lpNorm<Eigen::Infinity>());
...
// DOCTEST_TEST_CASE(
//   "sparse random strongly convex qp with equality and "
//   "inequality constraints: test changing default settings after several
//   solves " "using equality constrained initial guess")
// {
//   // std::cout << "---testing sparse random strongly convex qp with equality
//   and
//   // "
...
// TEST_CASE("ProxQP::dense: check ordering of z when there are box
// constraints")
// {
...
//     // osqp::dense::QP<T> qp_compare{ dim, n_eq, dim + n_in, false};
...
//                               qp_random.u) +
//        helpers::negative_part(qp_random.C * qp_compare.results.x -
//        qp_random.l))
//         .lpNorm<Eigen::Infinity>());
//     pri_res = std::max(pri_res,
...
//     // <= 3.02736e-05  (example) DOCTEST_CHECK(dua_res <= eps_abs); // Fail
//     here
...
//         qp_random.H, EigenValueEstimateMethodOption::ExactMethod, 1.E-6,
//         10000);
...
//     decltype(symmetric_mat)::Scalar>::epsilon());
...
//   osqp::dense::QP<T> qp(3, 0, 0);
//   qp.init(symmetric_mat, nullopt, nullopt, nullopt, nullopt, nullopt,
//   nullopt);
// }
...
// //   // std::cout << "---testing sparse random strongly convex qp with"
// //                "inequality constraints: test PrimalLDLT backend mu
// update---"
// //           //  << std::endl;
// //   double sparsity_factor = 1;
// //   utils::rand::set_seed(1);
...
// //   T pri_res = (helpers::negative_part(qp_random.C * qp.results.x -
// //   qp_random.l))
// //                 .lpNorm<Eigen::Infinity>();
// //   T dua_res = (qp_random.H * qp.results.x + qp_random.g +
// //                qp_random.C.transpose() * qp.results.z)
