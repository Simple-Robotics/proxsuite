...
TEST_CASE("ldlt: factorize compressed") {
	...
	*/
...

	{
		...
		// postorder: equivalente à la permutation de l'identité du point de vue du ratio de sparsité ;
		// garde meme structure matrice, mais change l'ordre pour que les parents de chaque noeud vienne après leur enfant
...
TEST_CASE("ldlt: factorize uncompressed, rank update") {
	...
	*/
	...
	factorize_numeric(
			...
			l_row_indices.ptr_mut(),
			nullptr,
			nullptr, // si on veut ajouter une diag à la matrice triangulaire supérieur on peut le faire avec diag_to_add et  la permuation direct perm; permutation direct
			l_col_ptrs.ptr(),
			etree.ptr(),
			perm_inv.ptr(),
			a,
	...
	// ld :matrice triangulaire inférieur avec les n valeurs de l et la diagonale remplacée par d
	MatMut<T, I> ld{
			from_raw_parts,
			n,
			n,
