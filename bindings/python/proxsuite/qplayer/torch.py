...
def QPFunction(eps=1e-9, maxIter=1000, eps_backward=1.E-4, structual_feasibility=True):
    ...
    class QPFunctionFn(Function):
        ...
        def forward(ctx, Q_, p_, A_, b_, G_, l_, u_):
            ...
            ctx.vector_of_qps = proxsuite.proxqp.dense.VectorQP()
            ctx.nBatch = nBatch
            ...
            proxsuite.proxqp.dense.solve_in_parallel(num_threads = ctx.cpu,qps=ctx.vector_of_qps)
            for i in range(nBatch):
        ...
        def backward(ctx, dl_dzhat):
            ...
            proxsuite.proxqp.dense.solve_backward_in_parallel(num_threads = ctx.cpu, qps=ctx.vector_of_qps,loss_derivatives=vector_of_loss_derivatives,eps=eps_backward)

            for i in range(nBatch):

                dQs[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_dH) 
                dps[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_dg)
                dGs[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_dC)
                dus[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_du) 
                dls[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_dl) 
                dAs[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_dA) 
                dbs[i] = torch.Tensor(ctx.vector_of_qps.get(i).model.backward_data.dL_db) 
    ...
    class QPFunctionFn_infeas(Function):
        ...
        def forward(ctx, Q_, p_, G_, h_, A_, b_):
            ...
            s_e = torch.Tensor(nBatch, ctx.neq).type_as(Q) \
                if ctx.neq > 0 else torch.Tensor()
            slacks = torch.Tensor(nBatch, ctx.nineq).type_as(Q)
            s_i = torch.Tensor(nBatch, ctx.nineq).type_as(Q) 
            
            vector_of_qps = proxsuite.proxqp.dense.VectorQP()

            ctx.cpu = os.cpu_count()
            if ctx.cpu is not None:
                ctx.cpu = max(1, int(ctx.cpu/2))
            l = -np.ones(ctx.nineq) * 1.E20
            ...
            proxsuite.proxqp.dense.solve_in_parallel(num_threads = ctx.cpu,qps=vector_of_qps)
        ...
        def backward(ctx, dl_dzhat, dl_ds_e, dl_ds_i):
            ...
            vector_of_qps = proxsuite.proxqp.sparse.VectorQP()
            ...
            for i in range(nBatch):
                ...
                P_1 = np.minimum(s_i,0.) + z_i >= 0. 
                P_2 = s_i <= 0. 
                P_2_c_s_i[i] = np.maximum(s_i,0.) # keep only (1-P_2)s_i for backward calculation afterward

                kkt[:dim,:dim] = Q_i 
                if neq>0:
                    kkt[:dim,dim:dim+n_eq] = A_i.transpose()
                    kkt[dim:dim+n_eq,:dim] = A_i 
                    kkt[dim+n_eq+n_in:dim+2*n_eq+n_in,dim:dim+n_eq] = -np.eye(n_eq)
                    kkt[dim+n_eq+n_in:dim+2*n_eq+n_in,dim+n_eq+2*n_in:2*dim+n_eq+2*n_in] = A_i
                ...
                kkt[dim+2*n_eq+n_in:,dim+n_eq:dim+n_eq+n_in] = -np.eye(n_in)
                kkt[dim+n_eq:dim+n_eq+n_in,dim+n_eq+n_in:dim+n_eq+2*n_in] = D_1_c
                kkt[dim+2*n_eq+n_in:,dim+n_eq+n_in:dim+n_eq+2*n_in] = -np.multiply(np.diag(D_1)[:,None],D_2)
                dim_ = 0 
                if n_eq >0:
                    dim_+=dim
                kkt[dim+2*n_eq+n_in:,dim+n_eq+2*n_in+dim_:] = np.multiply(np.diag(D_2_c)[:,None], C_i)
                ...
                qp = vector_of_qps.init_qp_in_place(H.shape[0], kkt.shape[0], C.shape[0])
            ...
            proxsuite.proxqp.sparse.solve_in_parallel(num_threads = ctx.cpu,qps=vector_of_qps)

            for i in range(nBatch):
                dx[i] = torch.from_numpy(np.float64(vector_of_qps.get(i).results.x[:dim]))
                if (n_eq>0):
                    dnu[i] = torch.from_numpy(np.float64(vector_of_qps.get(i).results.x[dim:dim+n_eq])) 
                dlam[i]= torch.from_numpy(np.float64(vector_of_qps.get(i).results.x[dim+n_eq:dim+n_eq+n_in]))
                dim_=0
                if n_eq>0:
                    b_5[i] = torch.from_numpy(np.float64(vector_of_qps.get(i).results.x[dim+n_eq+2*n_in:2*dim+n_eq+2*n_in]))
                    dim_+=dim
                b_6[i] = torch.from_numpy(np.float64(vector_of_qps.get(i).results.x[dim+n_eq+2*n_in+dim_:]))

            
            dps = dx
            dGs = bger(dlam.double(), zhats.double()) + bger(ctx.lams.double(), dx.double())+ bger(P_2_c_s_i.double(), b_6.double())
            if G_e:
                dGs = dGs.mean(0)
            dhs = -dlam
            if h_e:
                dhs = dhs.mean(0)
            if neq > 0:
                dAs = bger(dnu.double(), zhats.double()) + bger(ctx.nus.double(), dx.double()) + bger(s_e.double(), b_5.double())
                dbs = -dnu
                if A_e:
                    dAs = dAs.mean(0)
                if b_e:
                ...
                dAs, dbs = None, None
            dQs = 0.5 * (bger(dx.double(), zhats.double()) + bger(zhats.double(), dx.double()))
            if Q_e:
                dQs = dQs.mean(0)
            if p_e:
                dps = dps.mean(0)
