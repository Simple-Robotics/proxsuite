...
struct SparseLDLT {
	...
	isize lnnz =0;
	{
		...
		proxsuite::linalg::veg::dynstack::DynStackMut stack = proxsuite::linalg::veg::dynstack::DynStackMut{
...
#define PROX_QP_ANY_OF(...)                                                    \
  ::proxsuite::linalg::veg::dynstack::StackReq::or_(                           \
	...
	Eigen::Matrix<T, -1, -1, Eigen::ColMajor> l(){
		...
		Eigen::Matrix<T, -1, -1, Eigen::ColMajor> ld_eigen = to_eigen(ld.as_const());
		Eigen::Matrix<T, -1, -1, Eigen::ColMajor> l = ld_eigen.template triangularView<Eigen::UnitLower>();
		return l;
	}
	...
	Eigen::Matrix<T, -1, -1, Eigen::ColMajor> d(){
		...
		Eigen::Matrix<T, -1, -1, Eigen::ColMajor> ld_eigen = to_eigen(ld.as_const());
		Eigen::Matrix<T, -1, -1, Eigen::ColMajor> d = ld_eigen.diagonal().asDiagonal();
		return d;
	}

	Eigen::ArrayXi p(){
		Eigen::PermutationMatrix<-1, -1, I> perm_inv_eigen = to_eigen_perm(ldl.perm_inv.as_ref());
		Eigen::ArrayXi p = perm_inv_eigen.indices().array();
		return p;
	}
	Eigen::ArrayXi pt(){
		Eigen::PermutationMatrix<-1, -1, I> perm_inv_eigen = to_eigen_perm(ldl.perm_inv.as_ref()).inverse();
	...
	void solve_in_place(proxsuite::proxqp::sparse::VecRefMut<T> rhs_e){
			...
			auto ldl_col_ptrs = ldl.col_ptrs.ptr_mut();
			proxsuite::linalg::sparse::MatMut<T, I> ldlt = { // change name
				proxsuite::linalg::sparse::from_raw_parts,
				n_tot,
				n_tot,
				0,
...
exposeSparseLDLT(pybind11::module_ m)
{
  ...
  ::pybind11::class_<SparseLDLT<T,I>>(m, "SparseLDLT", pybind11::module_local())
	...
	.def("lt",
         &SparseLDLT<T,I>::lt,
         "Outputs the transpose of the lower triangular part of the sparse LDLT factorization "
		 "in dense format.")
	.def("d",
         &SparseLDLT<T,I>::d,
