...
struct QP {
	...
	void init(
		...
		proxsuite::proxqp::dense::update_proximal_parameters(results, work, rho, mu_eq, mu_in);
	...
	void solve(std::optional<VecRef<T>> x,
		...
		proxsuite::proxqp::dense::warm_start(x, y, z, results, settings);
		qp_solve( //
				settings,
				model,
				results,
...
*/
template <typename T>
proxqp::Results<T> solve(MatRef<T> H,
					 VecRef<T> g,
					 MatRef<T> A,
					 VecRef<T> b,
					 MatRef<T> C,
...
*/
template <typename T>
proxqp::Results<T> solve(const SparseMat<T>& H,
					 VecRef<T> g,
					 const SparseMat<T>& A,
					 VecRef<T> b,
					 const SparseMat<T>& C,
