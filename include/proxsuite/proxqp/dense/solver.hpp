...
auto primal_dual_newton_semi_smooth(
	...
	*/
	...
	for (i64 iter = 0; iter <= qpsettings.max_iter_in; ++iter) {
		...
		*/
		if (qpsettings.verbose ){
			std::cout << "\033[1;34m[inner iteration " << iter + 1  << "]\033[0m" << std::endl;
	...
	if (qpwork.dirty){ // the following is used when a solve has already been executed (and without any intermediary model update)
		switch (qpsettings.initial_guess) { 
					...
					case InitialGuessStatus::COLD_START_WITH_PREVIOUS_RESULT:{
						...
						ruiz.scale_primal_in_place({proxsuite::proxqp::from_eigen, qpresults.x});
						ruiz.scale_dual_in_place_eq({proxsuite::proxqp::from_eigen,qpresults.y});
						ruiz.scale_dual_in_place_in({proxsuite::proxqp::from_eigen,qpresults.z});
						break;
					}
					...
					case InitialGuessStatus::WARM_START:{
						...
						ruiz.scale_primal_in_place({proxsuite::proxqp::from_eigen, qpresults.x}); // it contains the value given in entry for warm start
						ruiz.scale_dual_in_place_eq({proxsuite::proxqp::from_eigen,qpresults.y});
						ruiz.scale_dual_in_place_in({proxsuite::proxqp::from_eigen,qpresults.z});
						break;
					}
		...
		if (qpsettings.initial_guess != InitialGuessStatus::WARM_START_WITH_PREVIOUS_RESULT){
				...
				proxsuite::proxqp::dense::setup_equilibration(qpwork, qpsettings, ruiz, false); // reuse previous equilibration
		...
		switch (qpsettings.initial_guess) {
			...
			case InitialGuessStatus::COLD_START_WITH_PREVIOUS_RESULT: {
				//!\ TODO in a quicker way
				ruiz.scale_primal_in_place({proxsuite::proxqp::from_eigen,qpresults.x}); // meaningful for when there is an upate of the model and one wants to warm start with previous result
				ruiz.scale_dual_in_place_eq({proxsuite::proxqp::from_eigen,qpresults.y}); 
				ruiz.scale_dual_in_place_in({proxsuite::proxqp::from_eigen,qpresults.z});
	...
	for (i64 iter = 0; iter < qpsettings.max_iter; ++iter) {
		...
		if (qpsettings.verbose) {
			...
			*/
			...
			std::cout << std::scientific << std::setw(2) << std::setprecision(2) << 
			"| primal residual=" << qpresults.info.pri_res << "| dual residual=" << qpresults.info.dua_res << " | mu_in=" << qpresults.info.mu_in << " | rho=" << qpresults.info.rho << std::endl;
