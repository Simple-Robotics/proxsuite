...
struct QP {
	...
	QP(const SparseMat<bool, I>& H,const SparseMat<bool,I>& A,const SparseMat<bool,I>& C)
			...
			proxsuite::linalg::sparse::MatRef<bool,I> Href = {proxsuite::linalg::sparse::from_eigen, H_triu};
			proxsuite::linalg::sparse::MatRef<bool,I> ATref = {proxsuite::linalg::sparse::from_eigen, AT};
			proxsuite::linalg::sparse::MatRef<bool,I> CTref = {proxsuite::linalg::sparse::from_eigen, CT};
			work.setup_symbolic_factorizaton(results,model,settings,preconditioner::RuizEquilibration<T, I>::scale_qp_in_place_req(proxsuite::linalg::veg::Tag<T>{}, _dim, _n_eq, _n_in),
	...
	void update(const std::optional<SparseMat<T, I>> H_,
		...
		proxsuite::linalg::sparse::MatMut<T, I> kkt_unscaled = model.kkt_mut_unscaled();

		auto kkt_top_n_rows = detail::top_rows_mut_unchecked(proxsuite::linalg::veg::unsafe, kkt_unscaled, n);
		...
		if (H_ != std::nullopt) {
			...
			if (A_ != std::nullopt) {
				if (C_ != std::nullopt) {
					bool res = have_same_structure(H_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,H_triu}) &&
						have_same_structure(AT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(A_.value().transpose())}) &&
						have_same_structure(CT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(C_.value().transpose())}) ;
					/* TO PUT IN DEBUG MODE
					std::cout << "have same structure = " << res << std::endl;
					*/
					if (res){
						copy(H_unscaled,{proxsuite::linalg::sparse::from_eigen,H_triu}); // copy rhs into lhs
						copy(AT_unscaled,{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(A_.value().transpose())}); // copy rhs into lhs
						copy(CT_unscaled,{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(C_.value().transpose())}); // copy rhs into lhs
					}
				} else {
					bool res = have_same_structure(H_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,H_triu})&&
						 have_same_structure(AT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(A_.value().transpose())});
				...
				*/
				if (res){
						copy(H_unscaled,{proxsuite::linalg::sparse::from_eigen,H_.value()}); // copy rhs into lhs
				}
			}
		} else if (A_ != std::nullopt) {
			if (C_ != std::nullopt) {
				bool res = have_same_structure(AT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(A_.value().transpose())})&&
					have_same_structure(CT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(C_.value().transpose())});
				/* TO PUT IN DEBUG MODE
				std::cout << "have same structure = " << res << std::endl;
				*/
			...
			bool res = have_same_structure(CT_unscaled.as_const(),{proxsuite::linalg::sparse::from_eigen,SparseMat<T,I>(C_.value().transpose())});
			/* TO PUT IN DEBUG MODE
			std::cout << "have same structure = " << res << std::endl;
			*/
			if (res){
	...
	void solve(std::optional<VecRef<T>> x,
		...
		proxsuite::proxqp::sparse::warm_start(x, y, z, results, settings);
		qp_solve( //
				results,
				model,
				settings,
