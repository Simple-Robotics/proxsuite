...
struct max2 {
	template <typename T>
	VEG_INLINE constexpr auto operator()(T const& a, T const& b) const
			-> T const& {
		return a > b ? a : b;
	}
};
...
template <typename T>
using OwnedMatrix = Eigen::Matrix<
		typename T::Scalar,
		Eigen::Dynamic,
		Eigen::Dynamic,
...
struct RowColAccessImpl<true> {
	...
	using Col = Eigen::Map<
			const_if<
					ptr_is_const<decltype(VEG_DECLVAL(T &&).data())>::value,
...
template <typename Mat>
auto trans(Mat&& mat) noexcept -> Eigen::Map< //
		_detail::const_if<
...
template <typename Mat>
auto diagonal(Mat&& mat) noexcept -> Eigen::Map< //
		_detail::const_if<
				_detail::ptr_is_const<decltype(mat.data())>::value,
				Eigen::Matrix< //
						typename veg::uncvref_t<Mat>::Scalar,
