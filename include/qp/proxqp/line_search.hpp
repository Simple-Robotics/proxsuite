...
auto gradient_norm_computation_box(
	...
	qpwork._err.tail(qpmodel._n_in) = ((qpwork._residual_in_z_u_plus_alpha.array() >= 0. && qpwork._active_part_z.array()>0.) || (qpwork._residual_in_z_u_plus_alpha.array() <= 0. && qpwork._active_part_z.array()<0.) ).select(qpwork._active_part_z, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));
	...
	qpwork._rhs.tail(qpmodel._n_in) = (qpwork._residual_in_z_u_plus_alpha.array() >= 0.).select(qpwork._primal_residual_in_scaled_u , Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in))
							+ (qpwork._residual_in_z_l_plus_alpha.array() <= 0.).select(qpwork._primal_residual_in_scaled_l , Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in))
							- (qpwork._residual_in_z_u_plus_alpha.array() < 0. && qpwork._residual_in_z_l_plus_alpha.array() > 0.).select(qpwork._active_part_z , qpwork._err.tail(qpmodel._n_in)) ;
	...
	for (isize k = 0; k < qpmodel._n_in; ++k) {
		...
		if (qpwork._residual_in_z_u_plus_alpha(k) >= 0.) {
			if (qpwork._active_part_z(k) > 0.) {
				qpwork._rhs.head(qpmodel._dim).noalias() += qpwork._active_part_z(k) * qpwork._c_scaled.row(k);
...
auto gradient_norm_qpalm_box(
	...
	qpwork._CTz = qpresults._rho * (qpresults._x - qpwork._xe) + qpwork._g_scaled;
	...
	T b(qpresults._x.dot(qpwork._d_dual_for_eq) + (qpwork._CTz).dot(qpwork._dw_aug.head(qpmodel._dim)) + ( qpwork._d_primal_residual_eq).dot(qpresults._y) ) ; 
	
	qpwork._rhs.tail(qpmodel._n_in) = (qpwork._residual_in_z_u_plus_alpha.array() > 0. || qpwork._residual_in_z_l_plus_alpha.array()<0.).select(qpwork._Cdx, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));
	a+= qpresults._mu_in * qpwork._rhs.tail(qpmodel._n_in).squaredNorm() ;

	qpwork._err.tail(qpmodel._n_in) = (qpwork._residual_in_z_u_plus_alpha.array() > 0.).select(qpwork._primal_residual_in_scaled_u, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in))
									  +(qpwork._residual_in_z_l_plus_alpha.array() < 0.).select(qpwork._primal_residual_in_scaled_l, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));

	b+= qpwork._rhs.tail(qpmodel._n_in).dot(qpwork._err.tail(qpmodel._n_in))*qpresults._mu_in;
	/*
	for (isize k = 0; k < qpmodel._n_in; ++k) {
...
auto local_saddle_point_box(
	...
	qpwork._rhs.head(qpmodel._dim) = qpwork._d_dual_for_eq;
	...
	// derive dz_act
	qpwork._rhs.tail(qpmodel._n_in) = ((qpwork._residual_in_z_u_plus_alpha.array() >= 0. && qpwork._active_part_z.array()>0.) || (qpwork._residual_in_z_u_plus_alpha.array() <= 0. && qpwork._active_part_z.array()<0.) ).select(qpwork._dw_aug.tail(qpmodel._n_in), Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));
	...
	// derive Cdx_act
	qpwork._err.tail(qpmodel._n_in) = ((qpwork._residual_in_z_u_plus_alpha.array() >= 0.) || (qpwork._residual_in_z_u_plus_alpha.array() <= 0.) ).select(qpwork._Cdx, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));
	...
	// derive z_act
	qpwork._rhs.tail(qpmodel._n_in) = ((qpwork._residual_in_z_u_plus_alpha.array() >= 0. && qpwork._active_part_z.array()>0.) || (qpwork._residual_in_z_u_plus_alpha.array() <= 0. && qpwork._active_part_z.array()<0.) ).select(qpwork._ze, Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in));
	...
	qpwork._active_part_z = (qpwork._residual_in_z_u_plus_alpha.array() >= 0.).select(qpwork._primal_residual_in_scaled_u , Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in))
							+ (qpwork._residual_in_z_l_plus_alpha.array() <= 0.).select(qpwork._primal_residual_in_scaled_l , Eigen::Matrix<T,Eigen::Dynamic,1>::Zero(qpmodel._n_in))
							- qpwork._rhs.tail(qpmodel._n_in) ; 
	
	c0 += qpwork._active_part_z.squaredNorm();

	b0 += qpwork._err.tail(qpmodel._n_in).dot(qpwork._active_part_z) ;
	...
	*/
 	for (isize k = 0; k < qpmodel._n_in; ++k) {
		...
		if (qpwork._residual_in_z_u_plus_alpha(k) >= 0) {
			...
			if (qpwork._active_part_z(k) > 0) {
				...
				qpwork._rhs.head(qpmodel._dim) += qpwork._dw_aug(qpmodel._dim+qpmodel._n_eq+k) * qpwork._c_scaled.row(k);
				qpwork._CTz += qpwork._ze(k) * qpwork._c_scaled.row(k);
	...
	b0 += qpwork._rhs.head(qpmodel._dim).dot(qpwork._CTz);
	b0 *= 2;
...
void initial_guess_LS(
	...
	//static constexpr T machine_eps  = 2.220446049250313e-16;
	...
	for (isize i = 0; i < qpmodel._n_in; i++) {
		if (std::abs(qpwork._ze(i)) != 0) {
			...
			if (std::abs(alpha_) < qpsettings._R) {
				...
				std::cout << "-z[i]/(dz[i]+sys.float_info.epsilon) : " << alpha_  << " z[i] " << qpwork._ze(i)<< " dz[i] " << qpwork._dw_aug(qpmodel._dim+qpmodel._n_eq+i)  <<  std::endl;
			}
		}
	}
	...
	//std::cout << " qpwork._primal_residual_in_scaled_u " << qpwork._primal_residual_in_scaled_u << std::endl;
	//std::cout << " qpwork._primal_residual_in_scaled_l " << qpwork._primal_residual_in_scaled_l << std::endl;
	for (isize i = 0; i < qpmodel._n_in; i++) {
		if (std::abs(qpwork._Cdx(i)) != 0) {
			alpha_ = -qpwork._primal_residual_in_scaled_u(i) / (qpwork._Cdx(i) + machine_eps);
			if (std::abs(alpha_) < qpsettings._R) {
				qpwork.alphas.push_back(alpha_);
				std::cout << "-residual_in_z_u[i]/(Cdx[i]+sys.float_info.epsilon) " << alpha_ << " -qpwork._primal_residual_in_scaled_u(i) "<<-qpwork._primal_residual_in_scaled_u(i) << " qpwork._Cdx(i) " << qpwork._Cdx(i) << std::endl;
			}
			alpha_ = -qpwork._primal_residual_in_scaled_l(i) / (qpwork._Cdx(i) + machine_eps);
			if (std::abs(alpha_) < qpsettings._R) {
				qpwork.alphas.push_back(alpha_);
				std::cout << "-residual_in_z_l[i]/(Cdx[i]+sys.float_info.epsilon) " << alpha_ << " -qpwork._primal_residual_in_scaled_l(i) "<<-qpwork._primal_residual_in_scaled_l(i) << " qpwork._Cdx(i) " << qpwork._Cdx(i) << std::endl;
			}
		}
	}
	...
	if (!qpwork.alphas.empty()) {
		...
		for (isize i = 0; i < n_alpha; ++i) {
			...
			T grad_norm = line_search::gradient_norm_computation_box(
					...
					qpmodel,
					alpha_);
			std::cout << "alpha_ " << alpha_ << " grad_norm " << grad_norm << std::endl;
		...
		for (isize i = -1; i < n_alpha ; ++i) {
			...
			//std::cout << "alpha_ local minimum " << alpha_ << " grad_norm " << associated_grad_2_norm << std::endl;
			
			// 3.4 if the argmin is within the interval [alpha[i],alpha[i+1]] is
			// stores the argmin and corresponding L2 norm

			if (i == -1) {
				std::cout << "alpha_ local minimum " << alpha_ << " grad_norm " << associated_grad_2_norm << " qpwork.alphas[0] " << qpwork.alphas[0] << std::endl;
				...
				std::cout << "alpha_ local minimum " << alpha_ << " grad_norm " << associated_grad_2_norm << " qpwork.alphas[n_alpha - 1 " << qpwork.alphas[n_alpha - 1] << std::endl;
				...
				std::cout << "alpha_ local minimum " << alpha_ << " grad_norm " << associated_grad_2_norm << " qpwork.alphas[i + 1] " << qpwork.alphas[i + 1]  << " qpwork.alphas[i ] " << qpwork.alphas[i ]<< std::endl;
...
auto gradient_norm_computation_box_old(
		VectorView<T> ze,
...
template <typename T, Layout LC>
auto local_saddle_point_box_old(
		VectorView<T> ze,
		VectorView<T> dz_,
		T mu_in,
		MatrixView<T, LC> C,
	...
	LDLT_MULTI_WORKSPACE_MEMORY(
			(d_dual_for_eq_aux_, Init, Vec(dim), LDLT_CACHELINE_BYTES, T), 
			(dual_for_eq_aux_, Init, Vec(dim), LDLT_CACHELINE_BYTES, T)
	);

	auto d_dual_for_eq_aux = d_dual_for_eq_aux_.to_eigen();
	auto dual_for_eq_aux = dual_for_eq_aux_.to_eigen();
	...
	d_dual_for_eq_aux = d_dual_for_eq;
	dual_for_eq_aux = dual_for_eq;
	...
	*/
	...
	for (isize k = 0 ; k < n_in ; ++k){
		...
		if (tmp_u(k) >= T(0.)){
			...
			if (active_part(k) > T(0)){
				...
				d_dual_for_eq_aux += dz(k) * C_copy.row(k) ; 
				dual_for_eq_aux += z_e(k) * C_copy.row(k) ; 
	...
	a0 += d_dual_for_eq_aux.squaredNorm() ; 
	c0 += dual_for_eq_aux.squaredNorm() ; 
	b0 += d_dual_for_eq_aux.dot(dual_for_eq_aux) ; 
	b0 *= T(2);
...
auto initial_guess_line_search_box(
	...
	for (isize i = 0; i < n_in; i++) {
		if (std::abs(z_e(i)) != 0) {
			if (std::abs(-z_e(i) / (dz_(i) + 2.220446049250313e-16) )<=5.){
				alphas.push_back(-z_e(i) / (dz_(i) + 2.220446049250313e-16));
			}
	...
	for (isize i = 0; i < n_in; i++) {
		if (std::abs(Cdx(i)) != 0) {
			if (std::abs(-residual_in_z_u(i) / (Cdx(i) + 2.220446049250313e-16))<=5.){
				alphas.push_back(-residual_in_z_u(i) / (Cdx(i) + 2.220446049250313e-16));
				//std::cout << "-residual_in_z_u(i) / (Cdx(i) + 2.220446049250313e-16) " << -residual_in_z_u(i) / (Cdx(i) + 2.220446049250313e-16) << std::endl;
			}
			
			if (std::abs(-residual_in_z_l(i) / (Cdx(i) + 2.220446049250313e-16))<=5.){
				alphas.push_back(-residual_in_z_l(i) / (Cdx(i) + 2.220446049250313e-16));
				//std::cout << "-residual_in_z_l(i) / (Cdx(i) + 2.220446049250313e-16) " << -residual_in_z_l(i) / (Cdx(i) + 2.220446049250313e-16) << std::endl;
			}
	...
	if (!alphas.empty()) {
		...
		for (auto a : alphas) {
			...
			if (std::abs(a) < T(1.e6)) {
				...
				// calcul de la norm du gradient du noeud
				T grad_norm = line_search::gradient_norm_computation_box_old(
						ze,
						dz_,
						mu_in,
						qp.C,
						...
						n_eq,
						n_in);
				//std::cout << "a " << a << " grad_norm " << grad_norm << std::endl;
		...
		for (isize i=0;i<n_;++i){
			std::cout << intervals[usize(i)] << std::endl;
		}
		for (isize i = 0; i < n_ - 1; ++i) {
			...
			//std::cout << "a_ before " << a_ << std::endl;
			// 3.3 on this interval the merit function is a second order
			// polynomial in alpha
			// the function "local_saddle_point_box" derives the exact minimum
			// and corresponding merit function L2 norm (for this minimum)
			T associated_grad_2_norm = line_search::local_saddle_point_box_old(
					ze,
					{from_eigen, dz_},
					mu_in,
					qp.C,
			...
			if (i == 0) {
				if (a_ <= intervals[1]) {
					...
					//std::cout << "intervals[1] " << intervals[1] <<  " a " << a_ << " associated_grad_2_norm " << associated_grad_2_norm << std::endl;
				}
			} else if (i == n_ - 2) {
				if (a_ >= intervals[usize(n_ - 2)]) {
					liste_norm_grad_interval.push_back(associated_grad_2_norm);
					liste_argmin.push_back(a_);
					//std::cout << "intervals[usize(n_ - 2)] " << intervals[usize(n_ - 2)]<< " a " << a_ << " associated_grad_2_norm " << associated_grad_2_norm << std::endl;
				}
			} else {
				if (a_ <= intervals[usize(i + 1)] && intervals[usize(i)] <= a_) {
					liste_norm_grad_interval.push_back(associated_grad_2_norm);
					liste_argmin.push_back(a_);
					//std::cout << " intervals[usize(i + 1)] " << intervals[usize(i + 1)] << " intervals[usize(i)] " << intervals[usize(i)] << " a " << a_ << " associated_grad_2_norm " << associated_grad_2_norm << std::endl;
				}
			}
		}
		...
		if (!liste_norm_grad_interval.empty()) {
			...
			/*
			if (!liste_norm_grad_noeud.empty()) {
				...
				auto index2 = std::min_element(
												vec_norm_grad_noeud.begin(), vec_norm_grad_noeud.end()) -
							vec_norm_grad_noeud.begin();
			
				if (vec_norm_grad_noeud[usize(index2)] < vec_norm_grad_interval[usize(index)]){
					alpha = vec_alphas[usize(index2)];
				}else{
...
template <typename T>
auto gradient_norm_qpalm_box_old(
		VectorView<T> x,
		VectorView<T> xe,
		VectorView<T> dx,
		T mu_eq,
...
auto correction_guess_line_search_box(
	...
	for (isize i = 0; i < n_in; i++) {
		if (Cdx(i) != 0) {
			//std::cout << "-residual_in_z_u(i) :" << -residual_in_z_u(i) <<" Cdx(i) : " << Cdx(i) <<  " machine_eps : " << machine_eps << " alpha : " << -residual_in_z_u(i) / (Cdx(i) + machine_eps)<< std::endl;
			alphas.push_back(-residual_in_z_u(i) / (Cdx(i) + 2.220446049250313e-16));
		}
		if (Cdx(i) != 0) {
			//std::cout << "-residual_in_z_l(i) :" << -residual_in_z_l(i) <<" Cdx(i) : " << Cdx(i) <<  " machine_eps : " << machine_eps << " alpha : " << -residual_in_z_l(i) / (Cdx(i) + machine_eps)<< std::endl;
			alphas.push_back(-residual_in_z_l(i) / (Cdx(i) + 2.220446049250313e-16));
		}
	}
		...
		for (auto a : alphas) {
			...
			if (a > machine_eps) {
				if (a < T(1.e21)) {
					 ...
					 * break the loop
					 */
					T gr = line_search::gradient_norm_qpalm_box_old(
							x,
							xe,
							dx,
							mu_eq,
...
void active_set_change_new(
	...
	qpwork._dw_aug.segment(qpmodel._dim,qpresults._n_c +qpmodel._n_eq).setZero();
}
