#ifndef INRIA_LDLT_OLD_LINE_SEARCH_HPP_2TUXO5DFS
#define INRIA_LDLT_OLD_LINE_SEARCH_HPP_2TUXO5DFS
...
auto old_gradient_norm_computation_box(
		VectorView<T> ze,
...
template <typename T>
auto old_gradient_norm_qpalm_box(
		VectorView<T> x,
		VectorView<T> xe,
		VectorView<T> dx,
		T mu_eq,
	...
	LDLT_MULTI_WORKSPACE_MEMORY(
			...
			(_active_set_tmp_u, Init, Vec(n_in), LDLT_CACHELINE_BYTES, bool),  //
			(_active_set_tmp_l, Init, Vec(n_in), LDLT_CACHELINE_BYTES, bool)  //
	);
	auto tmp_u = _tmp_u.to_eigen();
	auto tmp_l = _tmp_l.to_eigen();
	auto active_set_tmp_u = _active_set_tmp_u.to_eigen();
	auto active_set_tmp_l = _active_set_tmp_l.to_eigen();

	tmp_u = residual_in_z_u + Cdx * alpha;
	tmp_l = residual_in_z_l + Cdx * alpha;
	active_set_tmp_u = tmp_u.array() > 0;
	active_set_tmp_l = tmp_l.array() < 0;
	...
	LDLT_MULTI_WORKSPACE_MEMORY(
			(_tmp_a0_u, Init, Vec(num_active_u), LDLT_CACHELINE_BYTES, T), //
			(_tmp_b0_u, Init, Vec(num_active_u), LDLT_CACHELINE_BYTES, T),
			(_tmp_a0_l, Init, Vec(num_active_l), LDLT_CACHELINE_BYTES, T), //
			(_tmp_b0_l, Init, Vec(num_active_l), LDLT_CACHELINE_BYTES, T)
	);
...
template <typename T, Layout LC>
auto old_local_saddle_point_box(
		VectorView<T> ze,
		VectorView<T> dz_,
		T mu_in,
		MatrixView<T, LC> C,
	...
	//std::cout << "Ok old_local_saddle_point_box alloc before first loops" << std::endl;
	isize num_active_u = 0;
	isize num_active_l = 0;
	isize num_inactive = 0;
	for (isize k = 0; k < n_in; ++k) {
	...
	LDLT_MULTI_WORKSPACE_MEMORY(
			...
			(_active_set_l,Init, Vec(num_active_l),LDLT_CACHELINE_BYTES, isize),
            (_inactive_set,Init, Vec(num_inactive),LDLT_CACHELINE_BYTES, isize),
			(_z_p,Init, Vec(n_in),LDLT_CACHELINE_BYTES, T),
			(_dz_p,Init, Vec(n_in),LDLT_CACHELINE_BYTES, T),
			(_tmp_d2_u,Init, Vec(num_active_u),LDLT_CACHELINE_BYTES, T),
			(_tmp_d2_l,Init, Vec(num_active_l),LDLT_CACHELINE_BYTES, T),
            (_tmp_d3,Init, Vec(num_inactive),LDLT_CACHELINE_BYTES, T),
			(_tmp2_u,Init, Vec(num_active_u),LDLT_CACHELINE_BYTES, T),
			(_tmp2_l,Init, Vec(num_active_l),LDLT_CACHELINE_BYTES, T),
            (_tmp3,Init, Vec(num_inactive),LDLT_CACHELINE_BYTES, T)	
			);
	...
	//std::cout << "Ok old_local_saddle_point_box alloc active sets" << std::endl;
	
	// form the gradient
	//Eigen::Matrix<T, Eigen::Dynamic, 1> z_p(n_in);
	auto z_p = _z_p.to_eigen();
	z_p = z_e;
	//Eigen::Matrix<T, Eigen::Dynamic, 1> dz_p(n_in);
	auto dz_p = _dz_p.to_eigen();
	dz_p = dz;
	...
	//std::cout << "Ok old_local_saddle_point_box alloc z_p and dz_p" << std::endl;
	// a0 computation
	...
	*/
	...
	//std::cout << "Ok old_local_saddle_point_box a0 computation" << std::endl;
	// b0 computation
	for (isize k = 0; k < num_active_u; ++k) {
	...
	//std::cout << "Ok old_local_saddle_point_box b0 computation" << std::endl;
	// c0 computation
	T c0 = dual_for_eq.squaredNorm() + tmp2_u.squaredNorm() + tmp3.squaredNorm() +
	       tmp2_l.squaredNorm() + primal_residual_eq.squaredNorm();
	//std::cout << "Ok old_local_saddle_point_box c0 computation" << std::endl;
...
template <typename T>
auto old_initial_guess_line_search_box(
		VectorView<T> x,
		VectorView<T> y,
		VectorView<T> ze,
		VectorView<T> dw,
	...
	if (!alphas.empty()) {
		...
		for (auto a : alphas) {
			...
			if (std::abs(a) < 1.e6) {
				...
				T grad_norm = line_search::old_gradient_norm_computation_box(
						ze,
						dz_,
						mu_in,
						qp.C,
...
auto old_initial_guess_LS(
	...
	//std::cout << "ok alloc dz and z IG LS" << std::endl;
	T alpha = 1;
	...
	for (isize i = 0; i < n_in; i++) {
		if (std::abs(z_e(i)) != 0) {
			T alpha_ = -z_e(i) / (dz_(i) + machine_eps);
			if (std::abs(alpha_)< R){
	...
	for (isize i = 0; i < n_in; i++) {
		if (std::abs(Cdx(i)) != 0) {
			T alpha_= -residual_in_z_u(i) / (Cdx(i) + machine_eps);
			if (std::abs(alpha_) < R){
				alphas.push_back(alpha_);
			}
	...
	if (!alphas.empty()) {
		...
		for (isize i = 0; i < n_ - 1; ++i) {
			...
			// the function "local_saddle_point_box" derives the exact minimum
			// and corresponding merit function L2 norm (for this minimum)
			T associated_grad_2_norm = line_search::old_local_saddle_point_box(
					ze,
					dz,
					mu_in,
					C,
		...
		//std::cout << "ok old_local_saddle_point_box IG LS" << std::endl;
		///////// STEP 4 ///////////
		// if the list of argmin obtained from intervals is not empty the
		// algorithm return the one minimizing the most the merit function
		// Otherwise, it returns the node minimizing the most the merit
...
template <typename T>
auto old_correction_guess_line_search_box(
		VectorView<T> x,
		VectorView<T> xe,
		VectorView<T> ye,
		VectorView<T> ze,
		...
		for (auto a : alphas) {
			...
			if (a > machine_eps) {
				if (a < T(1.e21)) {
					 ...
					 * break the loop
					 */
					T gr = line_search::old_gradient_norm_qpalm_box(
							x,
							xe,
							dx,
							mu_eq,
...
#endif /* end of include guard INRIA_LDLT_OLD_LINE_SEARCH_HPP_2TUXO5DFS */
